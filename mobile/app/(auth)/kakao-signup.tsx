...
import { mobileKakaoSignup } from '../../src/lib/api';
...
export default function KakaoSignup() {
  ...
  const onSubmit = async () => {
    ...
    try {
      setLoading(true);
      const res = await mobileKakaoSignup({
        email: form.email,
        name: form.name,
        gender: form.gender,
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={[styles.row, { gap: 8, marginBottom: 8 }]}>
        ...
        <TouchableOpacity onPress={() => setAddrModal(true)} style={[styles.searchBtn]}>
          <Text style={{ fontWeight:'800' }}>검색</Text>
        </TouchableOpacity>
      </View>
      <TextInput
      ...
      <TouchableOpacity onPress={onSubmit} disabled={loading} style={[styles.submitBtn, loading && { opacity:0.7 }]}>
        {loading ? <ActivityIndicator /> : <Text style={styles.submitText}>카카오로 가입하기</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
...
function Pill({ text, selected, onPress }: { text:string; selected:boolean; onPress:()=>void }) {
  return (
    <TouchableOpacity onPress={onPress} style={[styles.pill, selected && { backgroundColor:'#181600', borderColor:'#181600' }]}>
...
const styles = StyleSheet.create({
  container: { padding:20, paddingTop:110, paddingBottom:20, backgroundColor:'#FFFBEA' },
  title: { fontSize:24, fontWeight:'900', marginBottom:18, color:'#181600' },
  label: { fontSize:14, color:'#3f3a1e', marginBottom:6 },
  input: {
    borderWidth:1, borderColor:'#f0d94a', borderRadius:12,
  ...
  pill: { paddingHorizontal:14, paddingVertical:10, borderRadius:999, borderWidth:1, borderColor:'#f0d94a', backgroundColor:'#fff' },
  pillText: { fontSize:14, color:'#181600', fontWeight:'700' },
  searchBtn: {
    height: INPUT_HEIGHT, paddingHorizontal:16, justifyContent:'center',
    borderRadius:10, backgroundColor:'#FEE500'
  },
  submitBtn: { marginTop:18, backgroundColor:'#FEE500', paddingVertical:14, borderRadius:12, alignItems:'center' },
  submitText: { fontSize:16, fontWeight:'900', color:'#181600' },
});
